...
(function() {
  ...
  /* ============ FIXED-BOTTOM VIEWPORT FIX (Android) ============ */
  /* Bug real (25/08, reportado por Andrés): en Android viejo, la barra de
     direcciones que se auto-oculta/muestra durante el scroll desincroniza
     el viewport de layout (el que usa position:fixed) del viewport visual
     real -- los elementos fijados abajo (bottom-nav, search-float, cartel
     de instalar PWA) se ven "empujados hacia abajo" y parcialmente tapados
     durante la animación. visualViewport da el tamaño REAL visible; la
     diferencia con innerHeight es el hueco que hay que compensar sumándolo
     al `bottom` de cada elemento fijo. Sin esto en navegadores sin soporte
     (--vv-bottom-gap queda en 0px, CSS ya trae ese fallback). */
  if (window.visualViewport) {
    const vv = window.visualViewport;
    const root = document.documentElement;
    function actualizarGapInferior() {
      const gap = Math.max(0, window.innerHeight - (vv.height + vv.offsetTop));
      root.style.setProperty('--vv-bottom-gap', gap + 'px');
    }
    vv.addEventListener('resize', actualizarGapInferior);
    vv.addEventListener('scroll', actualizarGapInferior);
...
(function() {
  ...
  window.addEventListener('load', function() {
    ...
    el.style.cssText = (_pwaDesk?'position:fixed;bottom:20px;right:16px;left:auto;max-width:340px;':'position:fixed;bottom:calc(80px + var(--vv-bottom-gap, 0px));left:16px;right:16px;')+'z-index:9999;display:flex;align-items:center;gap:12px;padding:14px 16px;background:#17181C;color:#F2EEE6;border-radius:16px;box-shadow:0 6px 32px rgba(0,0,0,0.65);font-family:var(--f-sans,sans-serif);font-size:15px;line-height:1.4;border:1px solid rgba(60,230,197,0.35);animation:pwaHintIn 0.35s ease;overflow:hidden';
